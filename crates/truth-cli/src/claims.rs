        ...
        for cand in candidates_in(&contents, file, config) {
            ...
            *counter += 1;
            ...
            specs.push(ClaimSpec {
                ...
                env: None,
                window: None,
                source: Some(ClaimSource { file: cand.file, line: Some(cand.line) }),
                extraction: Some(ClaimExtraction {
                    method: if config.llm.enabled { "llm".into() } else { "regex".into() },
...
pub fn claims(paths: &[String], out: Option<&str>, json: bool) -> Result<()> {
    ...
    if specs.is_empty() {
        if json {
            println!("{}", serde_json::to_string_pretty(&serde_json::json!({"claims": []}))?);
        } else {
    ...
    let claim_file = ClaimFile {
        ...
        defaults: ClaimDefaults { repo: Some(".".to_string()), window: Some("7d".to_string()), ..Default::default() },
        claims: specs,
    };
