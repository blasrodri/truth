...
fn setup() -> (rusqlite::Connection, Config) {
    ...
    truth_indexer::index_repo(&conn, &config.repo, Some(&repo_root().join("examples/sample-repo"))).unwrap();
    (conn, config)
}
...
fn check_json_is_valid_and_has_stable_keys() {
    ...
    let out = run_check(&conn, &config, "nobody uses /v1/checkout anymore", Trigger::Cli, Some(&sample_log())).unwrap();
    let j = out.to_json();
    // Re-serialize and parse to confirm it's valid JSON.
    let s = serde_json::to_string(&j).unwrap();
    let parsed: serde_json::Value = serde_json::from_str(&s).unwrap();
    for key in ["status", "confidence", "summary", "evidence", "caveats", "check_id"] {
        assert!(parsed.get(key).is_some(), "missing key {key}");
    }
    assert_eq!(parsed["status"], "contradicted");
    assert!(parsed["evidence"].as_array().unwrap().iter().any(|e| e["source"] == "local_logs"));
}
